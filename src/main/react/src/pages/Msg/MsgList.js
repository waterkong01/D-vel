      ...
      try {
        ...
        const updatedRooms = await Promise.all(
          rooms.map(async (room) => {
            const chatPartnerId =
              room.senderId === memberId ? room.receiverId : room.senderId;
            ...
            const name =
              nickNameRes.status === "fulfilled"
                ? nickNameRes.value.data
                : "알 수 없음";
            let img =
          ...
          <ChatUl>
            {chatRooms.length === 0 ? (
              ...
              chatRooms.map((room) => (
                ...
                >
                  <ProfileImg
                    src={roomImgs[room.roomId] || imgLogo2}
                    alt="Profile Img"
                  />
                  <div className="info">
                    <ChatName className="room_name">
                      {room.name || "로딩 중..."}
                    </ChatName>
                    <ChatName className="last_msg">
                      {room.lastMsg || "메시지 없음"}
                    </ChatName>
                  </div>
                </ChatRoom>
