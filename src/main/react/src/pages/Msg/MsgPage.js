...
const MsgPage = ({ setSelectedPage, darkMode }) => {
  ...
  const { roomId, roomNames, setRoomNames, roomImgs, setRoomImgs } =
    useContext(ChatContext); // 채팅방 번호, 새로운 방 개설, 기존 방 입장
        ...
        const getChatRoom = async () => {
          try {
            ...
            if (rsp) {
              ...
              const chatPartnerId =
                rsp.senderId === memberId ? rsp.receiverId : rsp.senderId;
              console.log("상대방의 memberId:", chatPartnerId);

              // 상대방의 닉네임 가져오기
              try {
                const nickNameRes = await ChattingApi.getNickNameByMemberId(
                  chatPartnerId
                );
              ...
              alert(
                "채팅방 정보를 불러올 수 없습니다. 이전 페이지로 이동합니다."
              );
            }
          } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!ws.current) {
      ...
      ws.current.onopen = async () => {
        ...
        if (sendMember === "") {
          // sender가 비어있을 경우에만 nickName을 가져와서 설정
    ...
    <ChattingRoomBg darkMode={darkMode}>
      ...
      <MessagesContainer ref={ChatContainerRef} darkMode={darkMode}>
        {chatList?.map((chat, index) => {
          const currentDate = new Date(chat.regDate)
            .toISOString()
            .split("T")[0]; // YYYY-MM-DD 형식
          const prevDate =
            index > 0
              ? new Date(chatList[index - 1].regDate)
                  .toISOString()
                  .split("T")[0]
              : null;
          ...
          return (
            <React.Fragment key={index}>
              {showDate && (
                <DateSeparator darkMode={darkMode}>
                  {formatDate(chat.regDate)}
                </DateSeparator>
              )}
              <MessageBox
                isSender={chat.sendMember === sendMember}
                darkMode={darkMode}
              >
                <Sender
                  isSender={chat.sendMember === sendMember}
                ...
                >
                  ...
                  <SentTime darkMode={darkMode}>
                    {chat.regDate
                      ? formatLocalDateTime(chat.regDate)
      ...
      <MsgInputBox>
        ...
        <SendButton
          src={
            "https://firebasestorage.googleapis.com/v0/b/ipsi-f2028.firebasestorage.app/o/firebase%2Fchaticon%2Fsend_color.png?alt=media"
          }
          alt="Send"
        ...
        <OverlayContainer>
          <OverlayContent>
            ...
            <BtnBox>
              ...
              <button className="submit" onClick={onClickMsgClose}>
                확인
              </button>
            </BtnBox>
          </OverlayContent>
